...
from .services.ocr_service import ocr_from_base64, ocr_from_screen_region
...
# OCR from screen region (uses PIL.ImageGrab)
...
async def perform_ocr_region(request: OCRRegionRequest):
    result = ocr_from_screen_region(request.x, request.y, request.width, request.height)
    if not result["success"]:
        raise HTTPException(status_code=500, detail=result.get("error", "OCR failed"))
    return {"text": result["text"], "blocks": result["blocks"]}
